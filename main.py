...
import os
import pandas as pd  # Mantenemos Pandas como estaba
import math
...
factor = None  # Inicializamos factor aquí para usarlo en tiempo real
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            if all(landmarks[i].visibility > umbral_visibilidad for i in idxs.values()):
                ...
                print(f"Frame {frame_id}: Posición (xm, ym) = ({xm}, {ym}), Velocidad (x, y) = ({vel_x:.6f}, {vel_y:.6f}), Aceleración (x, y) = ({acc_x:.6f}, {acc_y:.6f})")
                ...
                # Normalizar y dibujar vector de aceleración (ROJO)
                acc_length = math.sqrt(acc_x**2 + acc_y**2)
                if acc_length > MIN_ACC_THRESHOLD:
                    norm_acc_x = acc_x / acc_length
                    norm_acc_y = acc_y / acc_length
                    ...
                    print(f"Dibujando vector de aceleración en ({xm}, {ym}) -> ({end_point_acceleration[0]}, {end_point_acceleration[1]})")
...
if factor is not None:
    ...
    if len(df_visibles_metros) < 2:
        ...
        # Generación de gráficos para datos suavizados y sin suavizar
        ...
        for punto in puntos_seguir:
            ...
            fig_x_no_suav, axs_x_no_suav = plt.subplots(3, 1, figsize=(10, 8), sharex=True)
            fig_x_no_suav.suptitle(f'Movimiento de {punto.capitalize()} en eje X (Sin Suavizar)')
            axs_x_no_suav[0].plot(df_derivadas['frame'], df_derivadas[f'{punto}_x'], label='Posición', color='blue')
            axs_x_no_suav[0].set_ylabel('Posición (m)')
            axs_x_no_suav[0].legend()
            axs_x_no_suav[1].plot(df_derivadas['frame'], df_derivadas[f'{punto}_x_vel'], label='Velocidad', color='green')
            axs_x_no_suav[1].set_ylabel('Velocidad (m/s)')
            axs_x_no_suav[1].legend()
            axs_x_no_suav[2].plot(df_derivadas['frame'], df_derivadas[f'{punto}_x_acc'], label='Aceleración', color='red')
            axs_x_no_suav[2].set_xlabel('Frame')
            axs_x_no_suav[2].set_ylabel('Aceleración (m/s²)')
            axs_x_no_suav[2].legend()
            plt.tight_layout()
            ruta_grafico_x_no_suav = os.path.join(carpeta_datos, f'movimiento_{punto}_X_sin_suavizar.png')
            plt.savefig(ruta_grafico_x_no_suav)
            plt.close()
            ...
            fig_y_no_suav, axs_y_no_suav = plt.subplots(3, 1, figsize=(10, 8), sharex=True)
            fig_y_no_suav.suptitle(f'Movimiento de {punto.capitalize()} en eje Y (Sin Suavizar)')
            axs_y_no_suav[0].plot(df_derivadas['frame'], df_derivadas[f'{punto}_y'], label='Posición', color='purple')
            axs_y_no_suav[0].set_ylabel('Posición (m)')
            axs_y_no_suav[0].legend()
            axs_y_no_suav[1].plot(df_derivadas['frame'], df_derivadas[f'{punto}_y_vel'], label='Velocidad', color='orange')
            axs_y_no_suav[1].set_ylabel('Velocidad (m/s)')
            axs_y_no_suav[1].legend()
            axs_y_no_suav[2].plot(df_derivadas['frame'], df_derivadas[f'{punto}_y_acc'], label='Aceleración', color='brown')
            axs_y_no_suav[2].set_xlabel('Frame')
            axs_y_no_suav[2].set_ylabel('Aceleración (m/s²)')
            axs_y_no_suav[2].legend()
            plt.tight_layout()
            ruta_grafico_y_no_suav = os.path.join(carpeta_datos, f'movimiento_{punto}_Y_sin_suavizar.png')
            plt.savefig(ruta_grafico_y_no_suav)
            plt.close()

            # Gráficos en coordenadas polares (solo suavizados como ejemplo)
            fig_polar_suav, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), subplot_kw={'projection': 'polar'})
            fig_polar_suav.suptitle(f'Movimiento de {punto.capitalize()} en Polares (Suavizado)')
            ax1.plot(df_unificado[f'{punto}_ang'], df_unificado[f'{punto}_mag'], label='Posición', color='blue')
            ax1.set_title('Magnitud de Posición')
            ax1.legend()
            ax2.plot(df_unificado[f'{punto}_vel_ang'], df_unificado[f'{punto}_vel_mag'], label='Velocidad', color='green')
            ax2.set_title('Magnitud de Velocidad')
            ax2.legend()
            plt.tight_layout()
            ruta_grafico_polar_suav = os.path.join(carpeta_datos, f'movimiento_{punto}_polar_suavizado.png')
            plt.savefig(ruta_grafico_polar_suav)
            plt.close()
